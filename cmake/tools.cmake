...
function (axiom_configure_language_diagnostics target_name)
    ...
    set_property(
        TARGET "${target_name}"
        PROPERTY COMPILE_WARNING_AS_ERROR "${AXIOM_WARNINGS_AS_ERRORS}")
    ...
    elseif (CMAKE_CXX_COMPILER_ID MATCHES "Clang|GNU")
        ...
        if (AXIOM_DISABLE_MSVC_COMPATIBILITY AND CMAKE_CXX_COMPILER_ID MATCHES "Clang"
    ...
    if ("clang-tidy" IN_LIST AXIOM_STATIC_ANALYZERS)
        ...
        set_property(
            ...
            PROPERTY CXX_CLANG_TIDY
                     "${AXIOM_CLANG_TIDY_EXECUTABLE};--config-file=${PROJECT_SOURCE_DIR}/.clang-tidy;--warnings-as-errors=*")
    endif ()
...
function (axiom_configure_sanitizers target_name)
    ...
    test_san_flags(
        axiom_sanitizer_combination_available
        axiom_sanitizer_link_options
...
function (axiom_configure_tools)
    # Do not let the legacy cmake-scripts integration restore global sanitizer flags
    # from a build directory configured by an older Axiom revision.
    unset(USE_SANITIZER CACHE)

    # Clear cache variables written by the previous global analyzer integration.
    # Target properties configured above own analyzer activation from now on.
