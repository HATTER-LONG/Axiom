...
readMetadata(const py::handle& value) {
    ...
    for(const auto [meta_key, meta_value] : dictionary) {
        if(!PyUnicode_CheckExact(meta_key.ptr())) {
...
[[nodiscard]] InvocationContext contextFromPython(const py::object& context) {
    ...
    if(!PyDict_CheckExact(context.ptr())) {
 ...
 *
 * Every call converts Python data into owned C++ values and performs host
 * state coordination
 * while holding the GIL, then rebuilds fresh Python
 * objects before returning. The adapter
 * currently keeps the GIL throughout
 * dispatch because CommandDispatcher does not expose an
 * Action-only boundary.
 */
class Host {
public:
    Host(HostHandle handle, ErrorTypes types)
        : handle_(std::move(handle)), types_(std::move(types)) {}
    ...
    py::object dispatch(const py::object& method, /*method*/
                        const py::object& params, /*params*/
                        const py::object& context /*context*/) {
        try {
            if(!PyUnicode_CheckExact(method.ptr())) {
                throw py::type_error("method must be a str");
            }
            const auto method_name = py::cast<std::string>(method);
            if(!PyDict_CheckExact(params.ptr())) {
            ...
            const Result<Value> result =
                handle_.dispatch(method_name, params_value.asObject(), invocation_context);
            if(!result) {
                if(result.error().code == ErrorCode::HostClosed) {
                    raiseHostClosed(types_, result.error().message);
                }
                raiseAxiomError(types_, result.error());
            }
            return valueToPython(result.value());
        } catch(const py::error_already_set&) {
            throw;
        } catch(const ConversionError& error) {
            raiseConversionError(types_, error);
...
[[nodiscard]] Host attach(const HostHandle& handle, const ErrorTypes& types) {
    if(handle.empty()) {
        raiseHostClosed(types, "Cannot attach an empty host handle");
    }
    if(handle.closed()) {
        raiseHostClosed(types, "Cannot attach a closed host handle");
    }
    return Host{handle, types};
}
...
PYBIND11_MODULE(_axiom, module) {
    ...
    const auto error_types = axiom::python::initializeErrors(module);
    ...
    module.def(
        "attach",
        [error_types](const axiom::python::HostHandle& handle) {
            return axiom::python::attach(handle, error_types);
        },
        py::arg("handle"));
}
