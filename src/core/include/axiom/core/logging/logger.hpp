...
public:
    ...
    void writeFormatted(LogLevel level,
        ...
        try {
            write(level, std::format(format, std::forward<Arguments>(arguments)...), std::move(fields),
                  location);
        } catch(...) {
        }
    }
...
#define AXIOM_LOG(logger, level, fields, ...)                                                         \
    do {                                                                                               \
        ...
        if(axiom_log_logger_.enabled(axiom_log_level_)) {                                             \
            axiom_log_logger_.writeFormatted(axiom_log_level_, (fields), std::source_location::current(), \
                                             __VA_ARGS__);                                             \
        }                                                                                              \
    } while(false)
...
#define AXIOM_LOG_CRITICAL(logger, ...)                                                                \
    AXIOM_LOG((logger), ::axiom::core::logging::LogLevel::Critical, ::axiom::core::Value::Object{},  \
              __VA_ARGS__)
