 ...
 *
 * CMake defines AXIOM_CORE_BUILD_SHARED while producing a shared Axiom::Core
 * library and AXIOM_CORE_USE_SHARED for its consumers. Static builds leave the
 * annotation empty. The macro maps to the platform's native visibility mechanism
 * on Windows, Linux, and macOS.
 */

#if defined(_WIN32) || defined(__CYGWIN__)
#ifdef AXIOM_CORE_BUILD_SHARED
#define AXIOM_CORE_API __declspec(dllexport)
#elif defined(AXIOM_CORE_USE_SHARED)
#define AXIOM_CORE_API __declspec(dllimport)
#else
#define AXIOM_CORE_API
#endif
