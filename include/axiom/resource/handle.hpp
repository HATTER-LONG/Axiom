...
template <typename T> struct std::hash<axiom::resource::Handle<T>> {
    /**
     ...
     */
    [[nodiscard]] std::size_t operator()(const axiom::resource::Handle<T>& handle) const noexcept {
        return std::hash<axiom::resource::ResourceId>{}(handle.id());
    }
};
