...
def format_check(gate: Gate) -> bool:
    ...
    if not files:
        return gate.custom(
            "format", 10, lambda check: check.finish(False, "no source files found")
        )
...
def complexity_check(gate: Gate, files: Sequence[Path] | None = None) -> bool:
    ...
    return gate.command(
        "complexity", 10, ["lizard", "-l", "cpp", "-C", "10", "-L", "80", *paths]
    )
...
def _architecture(check: Check) -> bool:
    ...
    if not rules_path.is_file():
        return check.finish(
            False, "architecture rules are missing", expected=relative(rules_path)
        )
    ...
    for rule in rules.get("forbidden_include_prefixes", []):
        for file in files:
            ...
            for number, text in enumerate(
                ...
                if include and any(
                    include.group(1).startswith(item) for item in rule["forbidden"]
                ):
                    violations.append(
                        {
                            "rule": rule["id"],
